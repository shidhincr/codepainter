...
Tokenizer.prototype.end = function () {
	 ...
	 * syntax token
	 */
	function findGrammarToken( token, tree ){
		var elem;
		...
		for ( var key in tree ) {
			...
			if ( tree[ key ]['range'] !== undefined ) { // tree element has a range
				...
				if ( token['range'][0] >= tree[key]['range'][0] &&
					token['range'][1] <= tree[key]['range'][1]) { // token is within range of tree element
					
					elem = findGrammarToken(token, tree[key]);
    ...
    function nextToken() {
        ...
        if (pos < nextPos) {
            ...
            outToken = { type: 'Whitespaces', value: whitespaces, range: [ pos, nextPos ] };
            pos = nextPos;
        } else if (commentPos < tokenPos) {
            c++;
            pos = comment.range[1];
            var commentType = comment.type === 'Line' ? 'LineComment' : 'BlockComment';
            outToken = { type: commentType, value: comment.value, range: comment.range };
        } else {
			t++;
			pos = token.range[1];
			outToken = { type: token.type, value: token.value, range: token.range };
		}
