...
Transformer.prototype = {
	...
	infer : function(callback) {
		...
		codepainter.infer( this.options.infer, function(inferredStyle) {
			this.options.reporter(inferredStyle, 'Inferred style');
	...
	cascadeStyles : function() {
		...
		if (options.predef) {
			...
			options.reporter(style, predefStyle, options.predef + ' style');
			style = extend(style, predefStyle);
		}

		if (options.json) {
			var jsonStyle = require(options.json);
			options.reporter(style, jsonStyle, 'Supplied JSON file');
			style = extend(style, jsonStyle);
		}

		if (Object.keys(options.style).length) {
			options.reporter(style, options.style, 'Inline styles');
			style = extend(style, options.style);
		}

		if (options.editorConfig) {
			options.reporter(style, {
				'editor_config': true
	...
	inputEnd : function(err) {
		if( this.transformed + this.skipped === this.matched ) {
			this.callback( err, this.transformed, this.skipped );
		}
	},
