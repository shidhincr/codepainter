...
module.exports = util.inherits(Transformer, CodePainterObject, {
	...
	transformPath: function(inputPath, outputPath) {
		...
		inputStream.on('close', function() {
			if (outputPath) {
				fs.writeFile(outputPath, output.toString());
			}
		});
	},
	...
	inputEnd: function(err) {
		...
		err = this.firstError || err;
		this.emit('end', err, this.transformed, this.skipped, this.errored);
		if (typeof this.callback === 'function') {
			this.callback(err, this.transformed, this.skipped, this.errored);
		}
	},
