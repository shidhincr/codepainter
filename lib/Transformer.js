...
util.inherits(Transformer, EventEmitter, {
	...
	createStreams : function(input, rules, output) {
		...
		if( !output ) {
			...
			tokenizer.on('error', function(err) {
				this.onError.call(this, err, tempPath);
			}.bind(this));
		}
		var serializer = new Serializer();
		serializer.pipe(output);

		output.on('close', this.onTransformEnd.bind(this, inputPath, tempPath));
	...
	onError : function(err, tempPath) {
		if (tempPath) {
			fs.unlink(tempPath, function() {
				emitEnd.call(this, err);
			}.bind(this));
			return;
		}
		emitEnd.call(this, err);
		function emitEnd(err2) {
			this.emit('end', err2, this.transformed, this.skipped);
		}
	}
