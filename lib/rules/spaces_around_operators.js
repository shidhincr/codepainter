...
module.exports = util.inherits(SpacesAroundOperatorsRule, Rule, {
	...
	onTrueTransformData : function(token) {
		...
		if (prevToken) {
			if (this.shouldRemoveTokenSpace()) {
				this.token.value = '';
			} else if (this.shouldRemovePrevTokenSpace()) {
				prevToken.value = '';
			}
	...
	isOperatorAdjacentToNonspace : function() {
		if (this.hasOperatorThenNonspaces() && !this.isUnary(this.prevToken)) {
			return true;
		}
		if (this.hasNonspacesThenOperator() && !this.isUnary(this.token)) {
			return true;
		}
		return false;
	},
	...
	shouldRemovePrevTokenSpace : function() {
		return this.hasSpacesThenOperator() && this.isUnary(this.token);
	},
