...
IndentStyleAndSizeRule.prototype = extend({}, Rule.prototype, {
	...
	transform : function(input, settings, output) {
		...
		this.EOL = eol.getEOLChar(settings.end_of_line);
	...
	onTransformData : function(token) {
		...
		if(prevToken === null || this.hasNewline(prevToken)) {
			...
			//prevToken.value = this.EOL.repeat(lineCount) + this.indentOne.repeat(indent) + '/*' + (indent) + '(' + path + ')*/ ';
			prevToken.value = this.EOL.repeat(lineCount) + this.indentOne.repeat(indent);
		}
	...
	validate : function() {
		...
		assert(['tab', 'space'].indexOf(settings.indent_style) !== - 1);
		if(typeof settings.indent_size !== 'undefined') {
			assert(typeof settings.indent_size === 'number' || settings.indent_size === 'tab');
		}
	},
...
module.exports = IndentStyleAndSizeRule;
