	...
	input.on('data', function (token) {
		...
		if ( previousToken ) {
			if ( isOperator(token) ) {
				...
				if ( token.type === 'Whitespaces' && onlySpaces( token.value ) && ( shouldUseSpaces(token.value) === 'omitted' )  ) {
					// (token is whitespace) and (token is only spaces) and (spaces should be omitted)

					// drop token
					token.value = '';

				} else if ( (token.type !== 'Whitespaces') && ( shouldUseSpaces(token.value) === 'present' ) ){
					// (token is not whitespace) but (spaces should be present)
